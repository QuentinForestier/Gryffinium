...
public class UserController extends Controller
{
    ...
    public static class Signup
    {
        ...
        @Constraints.MinLength(value = 2, message = "Name must be at least 2 " +
                "characters")
        public String name;
        ...
        @Constraints.Required(message = "Password is required")
        @Constraints.ValidateWith(value = PasswordConstraint.class, message =
                "Password must be 8 characters, at least one digit, one " +
                        "lowercase, one uppercase, one special character")
        public String password;
    ...
    public Result login(Http.Request request)
    {
        Form<Login> loginForm =
                formFactory.form(Login.class).bindFromRequest(request);
        Form<Signup> signupForm =
                formFactory.form(Signup.class).bindFromRequest(request).discardingErrors();

        if (loginForm.hasErrors())
        {
            return badRequest(views.html.index.render(loginForm, signupForm,
                    request));
        }

        User user = userRepository.findByEmail(loginForm.get().email);

        if (user == null || !BCrypt.checkpw(loginForm.get().password,
                user.getPassword()))
        {
            loginForm = loginForm.withError("Credentials", "Invalid " +
                    "credentials");
            return badRequest(views.html.index.render(loginForm, signupForm,
                    request));
        }
    ...
    {
        Form<Signup> signupForm =
                formFactory.form(Signup.class).bindFromRequest(request);

        Form<Login> loginForm =
                formFactory.form(Login.class).bindFromRequest(request).discardingErrors();
        ...
        else if (!signupForm.get().password.equals(signupForm.get().confirmPassword))
        {
            ...
            return badRequest(views.html.index.render(loginForm,
                    signupForm.withError("Password", "Password and " +
